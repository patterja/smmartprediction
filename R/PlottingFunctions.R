...
bprle <- function (mat, title="RLE", ...){
  ...
  boxplot(rle, ..., outline=FALSE, las=2, main=title)
  abline(h = 0, lty = 2)
}
...
pcaplt_expset <- function(mat, metadat, title="PCA Plot"){
  #' PCA plot function with a metadata file and expression set
  #' 
  #' TODO: might be broken with metadata
  #' @param: mat (matrix): matrix
  #' @param: metadat (matrix): rownames correspond to colnames of mat
  ...
  PC1_and_PC2md =merge(PC1_and_PC2, md, by.x="type", by.y="adjnames", all.x)
  
  perc=(pca_prcomp$sdev^2)/sum(pca_prcomp$sdev^2) *100
  labs <- sapply(seq_along(perc), function(i) {paste("PC ", i, " (", round(perc[i], 2), "%)", sep = "")})
  
  
  p=ggplot(PC1_and_PC2md,aes(PC1, PC2, col=batch, shape=diagnosis)) + 
    geom_point(size=4) +
...
pcaplt_grid <- function(mat, md, title="PCA Plots"){
  ...
  PCs = data.frame(PC1=pca_prcomp$x[,1], PC2= pca_prcomp$x[,2], PC3= pca_prcomp$x[,3],type = rownames(pca_prcomp$x))
  PCs$batch = "bccl"
  PCs$batch[grep("HCC1954|T47D|SkBr3", PCs$type)] = "bccl_uhr"
  PCs$batch[grep("RNA170210CC|RNA171102CC|RNA170210CC", PCs$type)] = "smmart"
